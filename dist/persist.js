...
var TYPE_BACK_FORWARD = _browser.performance && _browser.performance.navigation.TYPE_BACK_FORWARD || 2;
...
function isBackForwardNavigated() {
	return _browser.performance && _browser.performance.navigation.type === TYPE_BACK_FORWARD;
}
...
_Persist2["default"].VERSION = "2.1.2-rc"; /**
...
Persist.StorageManager = _storageManager2["default"];
...
function getState() {
	...
	if (storage) {
		...
		if (_typeof(_browser.history.state) === "object" && _browser.history.state !== null) {
			stateStr = _browser.history.state[PERSIST_KEY];
		} else {
			warnInvalidStorageValue();
		}
	} else {
		stateStr = _browser.history.state;
	}
...
function setState(state) {
	...
	if (storage) {
		...
		try {
			var historyState = _browser.history.state === null ? {} : _browser.history.state;
